  ...
  if (
    ...
    a.exports = ((e) => {
      ...
      const h = (v) => {
        ...
        j = (v) => (1 == v ? "argument" : "arguments"),
        k = (v, w) => {
          return function (y, ...z) {
            if (z.length < w.minArgs)
              throw new Error(
                `Expected at least ${w.minArgs} ${j(w.minArgs)} for ${v}(), got ${z.length}`,
              );
            if (z.length > w.maxArgs)
              throw new Error(
                `Expected at most ${w.maxArgs} ${j(w.maxArgs)} for ${v}(), got ${z.length}`,
              );
            return new Promise((A, B) => {
              if (w.fallbackToNoCallback)
                try {
                  y[v](...z, i({ resolve: A, reject: B }, w));
                } catch (C) {
                  console.warn(
              ...
              get(B, C) {
                ...
                if (C in v) {
                  ...
                  if ("function" == typeof E) {
                    if ("function" == typeof w[C]) E = l(v, v[C], w[C]);
                    else if (m(x, C)) {
                      let F = k(C, x[C]);
                      E = l(v, v[C], F);
                    } else E = E.bind(v);
      ...
      const q = new g((v) => {
                ...
                const F = (G) => {
                  G.then(
                    ...
                    (H) => {
                      ...
                      (I =
                        H &&
                        (H instanceof Error || "string" == typeof H.message)
                          ? H.message
        ...
        s = (v, w, x, ...y) => {
          if (y.length < w.minArgs)
            throw new Error(
              `Expected at least ${w.minArgs} ${j(w.minArgs)} for ${v}(), got ${y.length}`,
            );
          if (y.length > w.maxArgs)
            throw new Error(
              `Expected at most ${w.maxArgs} ${j(w.maxArgs)} for ${v}(), got ${y.length}`,
            );
          return new Promise((z, A) => {
            const B = r.bind(null, { resolve: z, reject: A });
      ...
      return (
        (f.privacy = {
          network: { networkPredictionEnabled: u, webRTCIPHandlingPolicy: u },
          services: { passwordSavingEnabled: u },
          websites: { hyperlinkAuditingEnabled: u, referrersEnabled: u },
        }),
        n(e, t, f)
      );
    })(chrome);
